...
# === 启动哨兵：必须放在所有非 stdlib import 之前，定位 import/初始化阶段卡死 ===
# 仅依赖 stdlib，每行带毫秒时间戳，写入 output/startup_sentinel.log。
# 下次任务再卡死（如 2026-06-08 17:10 那次 30 分钟超时），看哨兵文件最后一条
# 停在哪一步，即可精准定位是哪个 import 或哪次 IO 阻塞了。
import sys as _sys
from datetime import datetime as _dt
from pathlib import Path as _Path

_SENTINEL = _Path(__file__).resolve().parent / "output" / "startup_sentinel.log"
_SENTINEL.parent.mkdir(exist_ok=True)

# 防止文件无限增长：超过 500KB 时截断保留最后 100KB
try:
    if _SENTINEL.exists() and _SENTINEL.stat().st_size > 500_000:
        _tail = _SENTINEL.read_bytes()[-100_000:]
        _SENTINEL.write_bytes(b"[truncated]\n" + _tail)
except Exception:
    pass


def _stamp(msg: str) -> None:
    """追加哨兵日志（毫秒精度）；在 logging 配置完成之前用于诊断阻塞点。"""
    try:
        with open(_SENTINEL, "a", encoding="utf-8") as f:
...
_stamp(f"=== python started, argv={_sys.argv} ===")
...
def _ensure_env_loaded():
    ...
    if _env_loaded:
        return
    _stamp("_ensure_env_loaded: importing dotenv")
    from dotenv import load_dotenv, find_dotenv
    _stamp("_ensure_env_loaded: searching .env")
    ...
    if _env_path:
        _stamp(f"_ensure_env_loaded: loading {_env_path}")
        load_dotenv(_env_path)
    _env_loaded = True
    _stamp("_ensure_env_loaded: done")
...
# ---------------------------------------------------------------------------
_stamp("before _ensure_env_loaded")
_ensure_env_loaded()
_stamp("before _setup_logging")
_setup_logging()

_stamp("before scripts imports")
from scripts.data_validator import run_check
_stamp("imported scripts.data_validator")
from scripts.feishu_notifier import send_to_feishu
_stamp("imported scripts.feishu_notifier")
from scripts.wiki_updater import get_tenant_token, update_wiki_content, delete_old_document
_stamp("imported scripts.wiki_updater")
