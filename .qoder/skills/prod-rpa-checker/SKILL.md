...
- 详版（按日期展开）：`- ❌ xx中心 / 网侧预测（D） — 10天内 3 天 load_type 缺 [1, 2]，命中 38/60`
- 汇总 4 计数：✅ 通过 / ❌ 失败 / ⚠️ 警告 / ⚙️ 配置缺失
- 详情节 3 节（条件显示）：### 失败详情 / ### 警告详情 / ### 配置缺失详情

> 详细技术方案见 [doc/网侧预测网侧实际load_type覆盖度校验技术方案.md](doc/网侧预测网侧实际load_type覆盖度校验技术方案.md)

#### 3.2 其他 8 种类型：10 天总条数显示

8 种非网侧类型（日前节点电价、实时节点电价、日前结算电价、实时结算电价、日前成交电量、代理用户用电明细、代理用户用电总计、明细与总计差异）的"数据量"列显示 `10 天 N 条`（N 是 10 天窗口内总行数）。

- **仅显示用**，**不进入**任何计数桶
- 最新数据时间校验**已**处理了"数据滞后"等失败情况
