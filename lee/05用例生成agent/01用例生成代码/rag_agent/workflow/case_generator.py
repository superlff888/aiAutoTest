...
# --------------------------------------------
# @FilePath    : lee\05用例生成agent\01用例生成代码\rag_agent\workflow\case_generator.py
# @Author      : Lee大侠
# @Desc        : 这是一个AI测试项目
...
1、从知识库中去检索功能相关的详细需求信息(agent中已经集成了知识库检索的能力) --> 封装为一个tool

2、用例生成的流程(将整个流程使用langgraph封装为一个workflow工作流，保障AI执行流程的完整性)--> 封装为一个tool：
     获取需求---> 大模型生成用例----> 对生成的用例进行逐条评审(并发)---> 覆盖率的检测(检测当前生成的用例是否覆盖了需求中所有的测试点，生成一个覆盖率检测的报告) ---> 存储用例(将生成的用例存储到文件，或者数据库)
...
【用例生成的完整流程】
1. 生成测试点：
获取需求--->基于需求整理测试点--->分析测试点是否覆盖需求中所有的功能，验证测试点覆盖率--->补全未覆盖的测试点--->输出所有的测试点
2. 生成测试用例：
